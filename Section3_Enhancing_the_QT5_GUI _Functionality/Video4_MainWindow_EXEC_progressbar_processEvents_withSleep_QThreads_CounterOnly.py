...
class MainWindow_EXEC():
    ...
    def get_thread_value(self, counter):
        print(counter)
...
class RunThread(QtCore.QThread):
    ...
    def run(self):
        while self.counter_start < 100 and self.isRunning == True:
            sleep(0.1)
            self.counter_start += 1
            print(self.counter_start)
            self.counter_value.emit(self.counter_start)     # emit new Signal with value
